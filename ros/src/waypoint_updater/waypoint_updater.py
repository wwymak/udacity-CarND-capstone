...
class WaypointUpdater(object):
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            if self.pose and self.base_waypoints:
                closest_wpt_idx = self.get_closest_waypoint_idx()
    ...
    def get_closest_waypoint_idx(self):
        ...
        closest_coord = self.waypoints_2d[closet_idx]
        prev_cord = self.waypoints_2d[closet_idx - 1]
        # check if waypoint is in front/behind:
        closest_wp_vect = np.array(closest_coord)
        prev_wp_vect = np.array(prev_cord)
        curr_vect = np.array([x, y])

        perpendicular_plane = np.dot(closest_wp_vect - prev_wp_vect, curr_vect - closest_wp_vect)
        if perpendicular_plane > 0:
